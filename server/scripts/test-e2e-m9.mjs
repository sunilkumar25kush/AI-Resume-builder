// M9 browser-path E2E via the Vite dev proxy — versions list + restore.
const BASE = "http://localhost:5173/api";

const reg = await fetch(`${BASE}/auth/register`, {
  method: "POST",
  headers: { "content-type": "application/json" },
  body: JSON.stringify({ name: "M9 Proxy", email: `m9p-${Date.now()}@test.com`, password: "password123" }),
});
...
const patch = (summary) =>
  fetch(`${BASE}/resumes/${resume._id}`, {
    ...
    body: JSON.stringify({ parsedData: { summary } }),
  });
await patch("First saved state");
await patch("Second saved state");

const list = await fetch(`${BASE}/resumes/${resume._id}/versions`, { headers: { cookie } });
const listJson = await list.json();
const versions = listJson.data?.versions ?? [];
console.log("versions via proxy:", list.status, "| count:", versions.length);

const v1 = versions.find((v) => v.version === 1);
const restore = await fetch(`${BASE}/resumes/${resume._id}/versions/${v1._id}/restore`, { method: "POST", headers: { cookie } });
const restored = await restore.json();
console.log("restore via proxy:", restore.status, "| summary:", restored.data?.resume?.parsedData?.summary);
